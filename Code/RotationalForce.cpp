...
// Modifier la déclaration dans la classe pour utiliser RigidbodyVolume
void RotationalForce::ResetDemo() {
	physicsSystem.ClearRigidbodys();
	physicsSystem.ClearConstraints();
...
void RotationalForce::Update(float dt) {
	...
	if (step) {
		...
		// Cast vers RigidbodyVolume pour accéder à box et AddRotationalImpulse
		RigidbodyVolume* body = static_cast<RigidbodyVolume*>(&bodies[0]);
		if (allowForceAdd && Raycast(body->box, ray, &result)) {
			body->AddRotationalImpulse(result.point, ray.direction * force);
			allowForceAdd = false;
		}
	}
}
